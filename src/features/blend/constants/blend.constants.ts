...
import type { RawMaterialAllocation } from "../../../shared/types/material.types";
import type {
  BlendDetail,
  BlendListRow,
  BlendPerWeekPoint,
...
/** Mock rows for /blend/list. Clicking a row's arrow opens BLEND_DETAILS[blendId]. */
export const BLEND_LIST: BlendListRow[] = Array.from({ length: 18 }, (_, index) => ({
  id: `bl-row-${index + 1}`,
  blendId: "BL-0001",
  productName: "Ceylon Blank Tea",
  blendName: "Celon Gold BLend",
  quantityKg: 40,
  allocationKg: 16,
}));

export const BLEND_LIST_ROWS_PER_PAGE = 10;

// ---- Blend detail ("view blend") modal -----------------------------------

const CEYLON_BLACK_TEA_MATERIALS: BlendDetail["materials"] = Array.from(
  { length: 14 },
  (_, index) => ({
    id: `mat-${index + 1}`,
...
/** Keyed by blendId so BlendListPage can look up which detail to show. */
export const BLEND_DETAILS: Record<string, BlendDetail> = {
  "BL-0001": {
    id: "BL-0001",
    productName: "Ceylon Black Tea",
    blendName: "Ceylon Gold Blend",
    quantityKg: 25,
    createdAt: "04 Jun 2026",
    status: "ACTIVE",
    materials: CEYLON_BLACK_TEA_MATERIALS,
  },
};

export const BLEND_DETAIL_MATERIALS_PER_PAGE = 8;

// ---- Create Blend modal ---------------------------------------------------

/** Options for the "Select Blend Name" dropdown in the Create Blend form. */
export const BLEND_NAME_OPTIONS = [
  "Ceylon Gold Blend",
  "Early Gray Blend",
  "Jasmine Green Blend",
  "White Tea Blend",
